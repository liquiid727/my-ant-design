# Playground 组件预览区（三列布局，复刻官网）

## Description

复刻 Ant Design 官网主题预览界面的 Components 展示区，严格按参考图（`assets/官网参考图.png`）的三列布局实现。左列：表单/数据录入组件；中列：数据展示/反馈组件；右列：业务场景卡片。所有组件包裹在 `ConfigProvider` 中，主题变化时实时更新。
...
Section 5.1 Playground Preview
