...
- [x] 高级主题目录结构：`src/themes/community/{theme-id}/meta.json` + `theme.ts` + `preview.png`
- [x] `meta.json` 包含与基础 JSON 主题相同的元数据字段，`format` 字段值为 `"advanced"`
- [x] `theme.ts` 导出 `useXxxTheme` hook，返回 `ConfigProviderProps`（与官方主题格式一致）
- [x] 高级主题可使用 `antd-style` 的 `createStyles` 编写自定义 CSS 类覆盖
- [x] 创建 `.github/workflows/compile-community-themes.yml`：
...
- [x] `CommunityThemeService.loadAdvancedTheme(themeId)` 通过 `import(GITHUB_PAGES_BASE + '/{themeId}.js')` 加载预编译模块
- [x] 加载失败时 wrap in try-catch，显示 "该高级主题加载失败" 提示
- [x] 提供高级主题模板（在 README 中）
