...
- [x] 创建 `src/themes/community/README.md`，内容包含：
  - 目录结构说明
  - 基础格式（JSON）详细文档 + 完整示例
  - 高级格式（TS）详细文档 + 完整示例
...
- [x] 创建 `.github/PULL_REQUEST_TEMPLATE/community-theme.md`，包含 checklist：
  - [ ] 主题 JSON 通过 schema 校验
  - [ ] 主题 ID 唯一（`{name}-{4位后缀}` 格式）
  - [ ] 预览图已上传（800×500, ≤200KB）
  - [ ] 在本地预览确认效果
- [x] 创建 2-3 个示例社区主题（JSON 格式），包含预览图：
  - 一个亮色系主题
  - 一个暗色系主题
  - （可选）一个品牌风格主题
- [x] 示例主题通过 JSON Schema 校验
