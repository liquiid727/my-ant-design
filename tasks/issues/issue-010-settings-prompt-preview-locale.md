...
- [x] 修改 `src/stores/settingsStore.ts`：`LLMConfig` 增加 `locale: 'zh-CN' | 'en-US'` 字段，默认值根据 `navigator.language` 自动选择
- [x] 向后兼容：已有 `ts_settings_llm` 缓存如无 `locale` 字段，fallback 到自动检测
- [x] 修改 `src/types.ts`：`LLMConfig` 类型增加 `locale` 字段
- [x] 修改 `src/components/settings/SettingsModal.tsx`：
...
- [x] Verify in browser using dev-browser skill：切换语言后 prompt 预览内容变化
